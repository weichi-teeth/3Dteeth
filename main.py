...
def run_emopt(emopt: EMOpt5Views, verbose: bool = False):
    ...
    for phtype in PHOTO_TYPES:
        ...
        canvas_pred_img.save(f"demo/int/{tag}/canvas_{tag}_0_{str(phtype.value)}.png")
    ...
    for phtype in PHOTO_TYPES:
        ...
        canvas_pred = canvas_pred + canvas_gt
        canvas_pred_img = Image.fromarray(canvas_pred)
        canvas_pred_img.save(f"demo/int/{tag}/canvas_{tag}_1_{str(phtype.value)}.png")
    ...
    for phtype in PHOTO_TYPES:
        ...
        canvas_pred = canvas_pred + canvas_gt
        canvas_pred_img = Image.fromarray(canvas_pred)
        canvas_pred_img.save(f"demo/int/canvas_{tag}_2_{str(phtype.value)}.png")
...
def main(tag="0"):
    ...
    print('hi3')
    # 保存模型预测轮廓线图
    if not os.path.exists(f"demo/int/{tag}"):
        os.makedirs(f"demo/int/{tag}")
    ...
    for phtype in PHOTO_TYPES:
        ...
        # edge_mask = np.load('/media/cx206/work/teeth/gt_temp/82YP6P/0343_82YP6P_下牙列像_segmentation.npy')
        edge_mask = gt_teeth_contour(edge_mask)
        cv2.imwrite(f"demo/int/{tag}/{tag}-{phtype.value}-label.png", edge_mask)
    ...
    for phtype in PHOTO_TYPES:
        ...
        try:
            ...
            canvas_pred = canvas_pred + canvas_gt
            canvas_pred_img = Image.fromarray(canvas_pred)
            canvas_pred_img.save(f"demo/int/{tag}/canvas_{tag}_pred_{str(phtype.value)}.png")
