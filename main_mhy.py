...
def run_emopt(emopt: EMOpt5Views, verbose: bool = False):
    ...
    for phtype in PHOTO_TYPES:
        ...
        canvas_pred_img.save(f"mhy/demo1019_newdata/int/{tag}/canvas_{tag}_0_{str(phtype.value)}.png")
    ...
    for phtype in PHOTO_TYPES:
        ...
        canvas_pred = canvas_pred + canvas_gt
        canvas_pred_img = Image.fromarray(canvas_pred)
        canvas_pred_img.save(f"mhy/demo1019_newdata/int/{tag}/canvas_{tag}_1_{str(phtype.value)}.png")
    ...
    for phtype in PHOTO_TYPES:
        ...
        canvas_pred = canvas_pred + canvas_gt
        canvas_pred_img = Image.fromarray(canvas_pred)
        canvas_pred_img.save(f"mhy/demo1019_newdata/int/canvas_{tag}_2_{str(phtype.value)}.png")
...
def main(tag="mhy"):
    ...
    print('hi3')
    # 保存模型预测轮廓线图
    if not os.path.exists(f"mhy/demo1019_newdata/int/{tag}"):
        os.makedirs(f"mhy/demo1019_newdata/int/{tag}")
    ...
    for phtype in PHOTO_TYPES:
        ...
        cv2.imwrite(f"mhy/demo1019_newdata/int/{tag}/{tag}-{phtype.value}-label.png", edge_mask)
    ...
    for phtype in PHOTO_TYPES:
        ...
        try:
            ...
            canvas_pred = canvas_pred + canvas_gt
            canvas_pred_img = Image.fromarray(canvas_pred)
            canvas_pred_img.save(f"mhy/demo1019_newdata/int/{tag}/canvas_{tag}_pred_{str(phtype.value)}.png")
...
if __name__ == "__main__":
    ...
    tags = []
    for subdirs, dirs, files in os.walk(file_root):
        for dir_name in dirs:
            tags.append(os.path.join(subdirs, dir_name))
