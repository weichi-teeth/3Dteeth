...
os.environ["CUDA_VISIBLE_DEVICES"] = "1,2,3"  # run on CPU
import sys
...
import psutil
import ray
####emopt5views_v2_jky 开始修改loss
import pcd_mesh_utils as pm_util
import recons_eval_metric as metric
from const_new import *
from emopt5views_jky import EMOpt5Views
from seg.seg_const import IMG_SHAPE
#from seg.seg_model import ASPP_UNet
from seg.utils_jky import predict_teeth_contour, gt_teeth_contour,gt_teeth_contour_jky,reset_by_indices_jky
import cv2
...
def run_emopt(emopt: EMOpt5Views, verbose: bool = False):
    ...
    for phtype in PHOTO_TYPES:
        ...
        canvas_pred_img.save(f"jky/demo/int/{tag}/canvas_{tag}_0_{str(phtype.value)}.png")
    ...
    for phtype in PHOTO_TYPES:
        ...
        canvas_pred = canvas_pred + canvas_gt
        canvas_pred_img = Image.fromarray(canvas_pred)
        canvas_pred_img.save(f"jky/demo/int/{tag}/canvas_{tag}_1_{str(phtype.value)}.png")
    ...
    for phtype in PHOTO_TYPES:
        ...
        canvas_pred = canvas_pred + canvas_gt
        canvas_pred_img = Image.fromarray(canvas_pred)
        canvas_pred_img.save(f"jky/demo/int/canvas_{tag}_2_{str(phtype.value)}.png")
    ...
    for it in range(stageIter[2]):
        emopt.maximization_step_5Views(stage, step=2, maxiter=maxiter, verbose=True)
...
def run_emopt_jky(emopt: EMOpt5Views, verbose: bool = False):
    # 3d teeth reconstruction by optimization
    print("-" * 100)
    print("Start optimization.")
...
def main(tag="0"):
    ...
    dir_Mu2 = "seg/train/500RealCases/Case" + tag + "/新病例阶段/ExportSTLs/"
    dir_Mu2_mid = "seg/train/500RealCases/Case" + tag + "/中期阶段1/ExportSTLs/"
    ...
    print('hi3')
    # 保存模型预测轮廓线图
    if not os.path.exists(f"jky/demo/int/{tag}"):
        os.makedirs(f"jky/demo/int/{tag}")
    ...
    for phtype in PHOTO_TYPES:
        ...
        #edge_mask = cv2.imread(f"seg/valid/label/{tag}-{phtype.value}.png", 0)####标签，5个文件，(1333, 2000, 2)
        edge_mask_npy_jky=np.load(f'/media/cx206/work/teeth/52393A_jky/{phtype.value}.npy')
        #2X1AJ0
        print('edge_mask',edge_mask_npy_jky.shape)
        #plt.imshow(edge_mask_npy_jky[:,:,0])
        #plt.show()
        print(edge_mask_npy_jky)
        edge_mask_jky = gt_teeth_contour_jky(edge_mask_npy_jky)
        #plt.imshow(edge_mask_jky[:, :, 0])
        #plt.show()
        cv2.imwrite(f"jky/demo/int/{tag}/{tag}-{phtype.value}-label.png", edge_mask_jky[:,:,0])
        ph=phtype.value
        if ph==0 or ph==1:
            flipped_image1 = np.flip(edge_mask_jky[:,:,0], axis=0)
            flipped_image2 = np.flip(edge_mask_jky[:,:,1], axis=0)
            edge_mask_jky[:,:,0]=flipped_image1
            edge_mask_jky[:,:,1]=flipped_image2
        if ph==2:
            edge_mask_jky=reset_by_indices_jky(edge_mask_jky, [11,41])
        if ph==3:
            edge_mask_jky=reset_by_indices_jky(edge_mask_jky, [21,31])
        if ph==4:
            edge_mask_jky=reset_by_indices_jky(edge_mask_jky, [15,16,45,46,25,26,35,36])
        

        edgeMasks.append(edge_mask_jky)
    ...
    for phtype in PHOTO_TYPES:
        ...
        canvas_gt = np.stack((canvas_gt, canvas_gt, canvas_gt), axis=-1)
        canvas_gt_img = Image.fromarray(canvas_gt)
        canvas_gt_img.save(f"jky/canvas_gt_{str(phtype.value)}.png")

        canvas_pred = (canvas_pred * 255)
        canvas_pred = np.stack((canvas_pred, np.zeros((600, 800)), np.zeros((600, 800))), axis=-1).astype(np.uint8)
        canvas_pred = canvas_pred + canvas_gt
        canvas_pred_img = Image.fromarray(canvas_pred)
        canvas_pred_img.save(f"jky/demo/int/{tag}/canvas_{tag}_pred_{str(phtype.value)}.png")
