...
from seg.utils_jky import reset_by_indices_predmask_jky


from const_new import PHOTO
...
class EMOpt5Views(object):
    def __init__(
        ...
        self.visIdx = [None] * 5
        self.edgeMask_teeth_number_jky= [None] * 5
        self.predMask_teeth_number_jky = [None] * 5
        print('#########################################################################################################################')
        for phType, eMask, visMask in zip(photoTypes, edgeMasks, visMasks):
            #assert eMask.ndim == 2, "edgeMask should be grayscale"  #cc
            assert eMask.ndim == 3, "new: edgeMask should be grayscale"  #
            print('eMask',eMask.shape,eMask)
            self.edgeMask[phType.value] = eMask[:,:,0]  # binary 2d-array
            self.edgeMask_teeth_number_jky[phType.value]=eMask[:,:,1]

            mask_number=[17,27,37,47]
            for number in mask_number:
                mask_to_zero=self.edgeMask_teeth_number_jky[phType.value]==number
                self.edgeMask[phType.value][mask_to_zero]=0
                self.edgeMask_teeth_number_jky[phType.value][mask_to_zero]=0
    ...
    def extractVisibleEdgePointsByAvgDepth(self, photoType):
        ...
        '''
        kcc=np.vstack(X_uv_sort)
        aaa=np.zeros((600,800))
        aaa[kcc[:,1],kcc[:,0]]=255
        over_lapping_area=np.logical_and(aaa,self.edgeMask[ph])
        result_image=np.where(over_lapping_area,255,0)
        ...
        '''
        if ph==0:
            tooth_order=np.array([ 6,  0,  7,  1,  2,  8,  3,  9,  4, 10,  5, 11])
        elif ph==1:
            tooth_order = np.array([ 0,  6,  1,  7,  2,  8,  9,  3, 10,  4,  5, 11])
        elif ph==2:
            tooth_order = np.array([ 2,  1,  3,  9,  8,  7,  4,  0, 10, 13, 11,  5, 12,  6])
        elif ph == 3:
            tooth_order = np.array([ 1,  2,  0, 10,  8,  9, 11,  3,  6,  7, 12,  4, 13,  5])
        elif ph == 4:
            tooth_order = np.array([ 0,  4, 12,  1,  8,  5,  9, 13, 14,  2, 10,  6, 15, 11,  3,  7])
        '''
        #if ph==4:
        #    print('tooth_order',tooth_order)
        #if ph==4 and tooth_order[0]==12:
        #    print(0)
        ...
        original_points=np.column_stack(np.where(self.edgeMask_teeth_number_jky[ph]>0))
        tree=KDTree(original_points)
        kc = np.vstack(vis_hull_vs)
        kc=kc[:,[1,0]]
        dintances,indices=tree.query(kc)
        thre=350
        matched_points=original_points[indices[dintances<thre]]
        #fdi_numbers=[self.edgeMask_teeth_number_jky[ph][idx[0],idx[1]] for idx in matched_points]
        fdi_numbers=[self.edgeMask_teeth_number_jky[ph][original_points[idx][0],original_points[idx][1]] for idx in indices]

        y_max=np.max(kc[:,0])
        x_max=np.max(kc[:,1])

        #fdi_image=np.zeros((600,800))
        fdi_image = np.zeros((y_max+1, x_max+1))

        #kc[kc >= 800] = 799
        #if ph==2 or ph==3 or ph==4:
        if ph == 5:
            for point,fdi in zip(kc,fdi_numbers):
                fdi_image[point[0],point[1]]=fdi

        if ph==1:
            for i,point_array in enumerate(vis_hull_vs):
                #point_array[point_array>=800]=799
                #point_array[:,0]=np.minimum(point_array[:,0],799)
                #point_array[:, 1] = np.minimum(point_array[:, 1], 599)
                vis_hull_vs[i]=point_array
                if i<6:
                    x, y =point_array[:, 0], point_array[:, 1]
                    fdi_image[y, x] = 31 + i
                else:
                    x, y = point_array[:, 0], point_array[:, 1]
                    fdi_image[y, x] = 41 + i-6
        ...
        if ph == 2:
            for i, point_array in enumerate(vis_hull_vs):
                ...
                if i == 0:
                    ...
                    x, y = point_array[:, 0], point_array[:, 1]
                    fdi_image[y, x] = 21 + i - 1
                elif i>=7 and i<=12:
                    x, y = point_array[:, 0], point_array[:, 1]
                    fdi_image[y, x] = 31 + i - 7
                elif i==13:
                    x, y = point_array[:, 0], point_array[:, 1]
                    fdi_image[y, x] = 41
        if ph == 3:
        ...
        if ph == 4:
            for i, point_array in enumerate(vis_hull_vs):
                ...
                if i >= 0 and i <= 3:
                    ...
                    x, y = point_array[:, 0], point_array[:, 1]
                    fdi_image[y, x] = 21+i-4
                elif i >= 8 and i <= 11:
                    x, y = point_array[:, 0], point_array[:, 1]
                    fdi_image[y, x] = 31+i-8
                elif i >= 12 and i <= 15:
                    x, y = point_array[:, 0], point_array[:, 1]
                    fdi_image[y, x] = 41 + i - 12
        count=1
        if ph==4:
            for ki in vis_hull_vs:
                a_img=np.zeros((y_max+1,x_max+1))
                #kii=np.array(ki)
                #ki[ki>=800]=799
                a_img[ki[:,1],ki[:,0]]=count
                count+=1
                plt.imshow(a_img)
                print(count)
        if ph==2:
            fdi_image=reset_by_indices_predmask_jky(fdi_image,[11,41])
        if ph==3:
            fdi_image=reset_by_indices_predmask_jky(fdi_image,[21,31])
        '''
        plt.imshow(fdi_image)
        ...
        if ph==0:
            self.count_0+=1
            print('########count_0',self.count_0)

        self.predMask_teeth_number_jky[ph]=fdi_image
        vis_hull_vids = [
            x
            for _, x in sorted(
    ...
    def updateEdgePrediction(self, photoType):
        ...
        #################3
        #np.save('points_vis/X_camera.npy', np.array(X_camera))
    ...
    def expectation(self, photoType, verbose, use_percentile=True):
        ...
        # get point correspondence between detected contour points and projected contour points
        ph = photoType.value
        ####self.P_true[ph] 404,2, P_pred  944,2
        point_loss_mat = (
        ...
        loss_mat = point_loss_mat * np.exp(normal_loss_mat) ###404,944 # weighted loss matrix
        _corre_pred_idx = np.argmin(loss_mat, axis=1)####404,
        losses = loss_mat[np.arange(self.M[ph]), _corre_pred_idx]#####404,
        if use_percentile == True:
            ...
            if ph==1:
                print(0)
            ####ph=1,self.flag_99_percentile[ph]  (1390,)
            self.flag_99_percentile[ph] = losses < np.percentile(losses, 99.0)####ph=0,(543,)
            ####ph=1,self.corre_pred_idx[ph]  (1376,)
            self.corre_pred_idx[ph] = _corre_pred_idx[
                self.flag_99_percentile[ph]
            ]  ##########################_ self.corre_pred_idx[ph],(537,),,corre_pred_idx,(543,)
            # 找到使每个真实点的损失最小化的预测点的索引
            ####self.P_true_99_percentile[ph] (1376,2)
            self.P_true_99_percentile[ph] = self.P_true[ph][self.flag_99_percentile[ph]]####537,2
            ####
            self.loss_expectation_step[ph] = np.sum(losses[self.flag_99_percentile[ph]])
        else:
            self.corre_pred_idx[ph] = _corre_pred_idx#####404,
...
######################jky修改
    def expectation(self, photoType, verbose, use_percentile=True):
        ...
        Get point correspondence between detected contour points and projected contour points
        grouped by category.
        :param photoType: PhotoType enum indicating the type of photo
        :param verbose: Boolean flag to print detailed information
        :param use_percentile: Boolean flag to use 99 percentile for filtering
        :return: None
        """
        ph = photoType.value
        categories = np.unique(self.edgeMask_teeth_number_jky[ph])
        all_losses = []
        num_points = len(self.P_true[ph])
        if use_percentile==False:
            self.corre_pred_idx[ph] = np.zeros(num_points, dtype=int)
            self.flag_99_percentile[ph] = np.zeros(num_points, dtype=int)
            self.P_true_99_percentile[ph]=np.zeros_like(self.P_true[ph])
        #print('aaaaaaaaaaaaaaaa',ph,self.P_pred[ph])
        if use_percentile:
            flag_99_percentile_list = []
            corre_pred_idx_list = []
            P_true_99_percentile_list = []
            loss_expectation_step_list = []
            losses_list=[]
            _corre_pred_idx_list=[]
            cat_points_list=[]
            cat_normals_list=[]
            cat_points_pred_list=[]
            cat_normals_pred_list=[]


        for cat in categories:
            if cat==0:
                continue
            if cat==17 or cat==27 or cat==37 or cat==47:
                continue
            # Create mask for current category
            #cat_mask = (self.edgeMask_teeth_number_jky[ph] == cat)
            cat_mask=np.array([self.edgeMask_teeth_number_jky[ph][int(y),int(x)]==cat for x,y in self.P_true[ph]])

            #self.P_pred[ph][self.P_pred[ph]>=800]=800-1
            cat_mask_pred = np.array([self.predMask_teeth_number_jky[ph][int(y), int(x)] == cat for x, y in self.P_pred[ph]])

            # Points and normals for the current category
            cat_points = self.P_true[ph][cat_mask]
            cat_normals = self.P_true_normals[ph][cat_mask]

            cat_points_pred=self.P_pred[ph][cat_mask_pred]
            cat_normals_pred=self.P_pred_normals[ph][cat_mask_pred]
            if use_percentile:
                cat_points_list.append(cat_points)
                cat_normals_list.append(cat_normals)
                cat_points_pred_list.append(cat_points_pred)
                cat_normals_pred_list.append(cat_normals_pred)

            # Compute distance matrix and normal loss matrix for current category
            #if ph==2 or ph==3 or ph==4:
            if ph == 7:#
            #if ph == 2 or ph == 3 or ph == 4 or ph==1 or ph==0:
                point_loss_mat = (
                        distance_matrix(cat_points, self.P_pred[ph], p=2, threshold=1e8) ** 2
                )
                normal_loss_mat = (
                        -((cat_normals @ self.P_pred_normals[ph].T) ** 2) / self.varAngle
                )
            else:
                print(f'##########################!!{ph}')
                if len(cat_points_pred) == 0:
                    print('cat_points=',0)
                point_loss_mat = (
                        distance_matrix(cat_points, cat_points_pred, p=2, threshold=1e8) ** 2
                )
                normal_loss_mat = (
                        -((cat_normals @cat_normals_pred.T) ** 2) / self.varAngle
                )


            # Compute weighted loss matrix
            loss_mat = point_loss_mat * np.exp(normal_loss_mat)
            _corre_pred_idx = np.argmin(loss_mat, axis=1)
            losses = loss_mat[np.arange(len(cat_points)), _corre_pred_idx]
            if use_percentile:
                _corre_pred_idx_list.append(_corre_pred_idx)
                losses_list.append(losses)
            # Apply percentile filtering if required
            #use_percentile=True

            '''  
            if use_percentile:
                print('!!!!!!99')
                flag_99_percentile = losses < np.percentile(losses, 99.0)
                #_corre_pred_idx = _corre_pred_idx[flag_99_percentile]
                #self.flag_99_percentile[ph][cat_mask] = flag_99_percentile
                #self.P_true_99_percentile[ph][cat_mask] = self.P_true[ph][flag_99_percentile]
                #cat_points = cat_points[flag_99_percentile]

                # 更新百分位标记数组，先创建一个与 P_true 相同长度的全 False 数组
                full_flag_99_percentile = np.zeros(len(self.P_true[ph]), dtype=bool)
                # 在 cat_mask 对应的位置上设置 flag_99_percentile 的值
                full_flag_99_percentile[cat_mask] = flag_99_percentile
                # 存储更新后的 flag_99_percentilea
                self.flag_99_percentile[ph] = full_flag_99_percentile
                # 根据 full_flag_99_percentile 更新 P_true_99_percentile
                #self.P_true_99_percentile[ph][cat_mask]= self.P_true[ph][self.flag_99_percentile[ph]]
                combined_maks2=cat_mask & self.flag_99_percentile[ph]
                self.P_true_99_percentile[ph][combined_maks2] = self.P_true[ph][combined_maks2]

                # 仅选出低损失的预测索引
                filter_indcies=_corre_pred_idx[flag_99_percentile]
                combined_mask=np.zeros_like(cat_mask,dtype=bool)
                combined_mask[cat_mask]=flag_99_percentile
                #self.corre_pred_idx[ph][cat_mask] = _corre_pred_idx[flag_99_percentile]
                self.corre_pred_idx[ph][combined_mask]=filter_indcies

                total_loss = np.sum(losses[flag_99_percentile])
                '''
            ...
            if use_percentile == True:
                ...
                flag_99_percentile= losses < np.percentile(losses, 99.0)
                flag_99_percentile_list.append(flag_99_percentile)
                ####ph=1,self.corre_pred_idx[ph]  (1376,)
                #self.corre_pred_idx[ph] = _corre_pred_idx[
                #    self.flag_99_percentile[ph]
                #]
                corre_pred_idx=_corre_pred_idx[flag_99_percentile]
                corre_pred_idx_list.append(corre_pred_idx)
                # 找到使每个真实点的损失最小化的预测点的索引
                ####self.P_true_99_percentile[ph] (1376,2)
                #self.P_true_99_percentile[ph] = self.P_true[ph][self.flag_99_percentile[ph]]  ####537,2
                P_true_99_percentile=cat_points[flag_99_percentile]
                P_true_99_percentile_list.append(P_true_99_percentile)
                ####
                #self.loss_expectation_step[ph] = np.sum(losses[self.flag_99_percentile[ph]])
                loss_expectation_step=np.sum(losses[flag_99_percentile])
                total_loss=loss_expectation_step
                loss_expectation_step_list.append(loss_expectation_step)
            else:
                self.corre_pred_idx[ph][cat_mask] = _corre_pred_idx
            ...
            #print(ph,self.loss_expectation_step[ph])
            #self.loss_expectation_step[ph] += total_loss
            #if total_loss>5:
            #    total_loss=5
            ...
            if verbose:
                print("{} - Category: {} - Unique pred points: {} - E-step loss: {:.2f}".format(
                    str(photoType), cat, len(np.unique(_corre_pred_idx)), total_loss
                ))
        part=False
        if use_percentile and part== True:
            #self.flag_99_percentile[ph]=np.vstack(flag_99_percentile_list)
            self.flag_99_percentile[ph]=np.concatenate(flag_99_percentile_list)
            #self.corre_pred_idx[ph]=np.vstack(corre_pred_idx_list)
            self.corre_pred_idx[ph]=np.concatenate(corre_pred_idx_list)
            self.P_true_99_percentile[ph]=np.vstack(P_true_99_percentile_list)
            self.loss_expectation_step[ph]=np.sum(loss_expectation_step_list)
            print('99loss',self.loss_expectation_step[ph])
        elif use_percentile and part== False:
            #p_true=np.vstack(cat_points_list)
            #p_pred=np.vstack(cat_points_pred_list)
            ...
            #_corre_pred_idx=np.concatenate(_corre_pred_idx_list)
            self.flag_99_percentile[ph] = losses < np.percentile(losses, 99.0)  ####ph=0,(543,)
            ####ph=1,self.corre_pred_idx[ph]  (1376,)
            self.corre_pred_idx[ph] = _corre_pred_idx[
                self.flag_99_percentile[ph]
            ...
            self.loss_expectation_step[ph] = sum(all_losses)

        if verbose:
            print("Total loss for photo type {}: {:.2f}".format(str(photoType), sum(all_losses)))
            print(f"合计损失：{self.loss_expectation_step[ph]}")
