...
def computeRMSD(X_Pred, X_Ref, return_list=False):
    ...
    for x_pred, x_ref in zip(X_Pred, X_Ref):
        ...
        rmsd1 = np.sqrt(np.mean(squared_sd1))
        rmsd2 = np.sqrt(np.mean(squared_sd2))
        RMSDs.append(rmsd)
        RMSD1s.append(rmsd1)
        RMSD2s.append(rmsd2)
        time.sleep(1)
    ...
    if return_list == True:
        return RMSDs
    return (np.mean(RMSDs), np.mean(RMSD1s), np.mean(RMSD2s), RMSDs, RMSD1s, RMSD2s)
